...
type NodePeerResolver struct {
	client *clientv3.Client
}

var _ domain.NodePeerResolver = (*NodePeerResolver)(nil)

func NewNodePeerResolver(client *clientv3.Client) *NodePeerResolver {
	return &NodePeerResolver{
		client: client,
	}
}

func (r *NodePeerResolver) Resolve(ctx context.Context, node domain.NodeID) (*domain.NodePeer, error) {
	key := fmt.Sprintf("/alive/nodes/%010d", uint32(node))
	...
	if err := protojson.Unmarshal([]byte(resp.Kvs[0].Value), &msg); err != nil {
		return nil, err
	}
